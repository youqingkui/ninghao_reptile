...
(function() {
  var async, cheerio, down, downUrlArr, fs, loginCookie, ninghao, request;
  ...
  ninghao = function(cookie, urlArr) {
    this.cookie = cookie;
    this.urlArr = urlArr;
    this.href = [];
    this.info = {};
  ...
  ninghao.prototype.parseUrlArr = function() {

    /* 解析课程链接数组 */
    var self;
    self = this;
    return async.eachSeries(self.urlArr, function(item, callback) {
      console.log(item);
      return self.getUrl(item, callback);
    }, function(eachErr) {
      if (eachErr) {
  ...
  ninghao.prototype.getUrl = function(url, cb) {
    ...
    return request.get(op, function(err, res, body) {
      ...
      return self.doTask(cb);
    });
  };

  ninghao.prototype.doTask = function(cb) {
  ...
  ninghao.prototype.getDownInfo = function(url, cb) {
    ...
    return request.get(op, function(err, res, body) {
      ...
      if (!$('script:contains("这不是秘密哦")').length) {
  ...
  downUrlArr = ['http://ninghao.net/course/1584', 'http://ninghao.net/course/1479', 'http://ninghao.net/course/1444'];
